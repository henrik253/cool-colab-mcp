...
async def capture_document(session: NotebookSession) -> dict[str, Any]:
    ...
    return session.merge_cached_outputs(
        notebook_document(_cells(result.structured_content))
    )
...
def register_snapshot_tools(mcp: FastMCP, sessions: SessionManager) -> None:
    ...
    async def capture(
        ...
        return session.notebook_id, session.merge_cached_outputs(
            notebook_document(_cells(result.structured_content), recovery)
        )
